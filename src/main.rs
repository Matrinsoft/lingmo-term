...
fn print_help() {
    println!(
        r#"COSMIC Terminal
Designed for the COSMIC鈩?desktop environment, cosmic-term is a liblingmo-based terminal emulator.

Project home page: https://github.com/Matrinsoft/lingmo-term
Options:
    ...
    fn update_title(&mut self, pane: Option<pane_grid::Pane>) -> Task<Message> {
        ...
        if let Some(tab_model) = self.pane_model.panes.get(pane) {
            let (header_title, window_title) = match tab_model.text(tab_model.active()) {
                Some(tab_title) => (
                    tab_title.to_string(),
                    format!("{tab_title} 鈥?{}", fl!("cosmic-terminal")),
                ),
...
impl Application for App {
    ...
    const APP_ID: &'static str = "com.lingmoos.LingmoTerm";
    ...
    fn init(mut core: Core, flags: Self::Flags) -> (Self, Task<Self::Message>) {
        ...
        let about = About::default()
            ...
            .links([
                (fl!("repository"), "https://github.com/Matrinsoft/lingmo-term"),
                (
                    fl!("support"),
                    "https://github.com/Matrinsoft/lingmo-term/issues",
                ),
            ]);
